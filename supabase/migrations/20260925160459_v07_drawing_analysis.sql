-- v0.7: private advisory drawing analysis; attempts, answers and deterministic grades are untouched.
...
alter table public.ai_requests drop constraint if exists ai_requests_feature_cost_check;
alter table public.ai_requests add constraint ai_requests_feature_check
  check (feature in ('hint', 'explain_mistake', 'explain_solution', 'calculation_grading', 'drawing_analysis'));
alter table public.ai_requests add constraint ai_requests_reasoning_effort_check
  check ((feature in ('calculation_grading', 'drawing_analysis') and reasoning_effort = 'medium')
    or (feature in ('hint', 'explain_mistake', 'explain_solution') and reasoning_effort = 'low'));
alter table public.ai_requests add constraint ai_requests_feature_cost_check
  check (credits = case feature
    ...
    when 'explain_solution' then 2 when 'calculation_grading' then 2
    when 'drawing_analysis' then 4 end);
...
begin
  ...
  return jsonb_build_object('limit', 20, 'used', v_used, 'remaining', greatest(0, 20 - v_used),
    ...
    'featureCosts', jsonb_build_object('hint', 1, 'explain_mistake', 1,
      'explain_solution', 2, 'calculation_grading', 2, 'drawing_analysis', 4));
end $$;
...
begin
  v_credits := case p_feature when 'hint' then 1 when 'explain_mistake' then 1
    when 'explain_solution' then 2 when 'calculation_grading' then 2 when 'drawing_analysis' then 4 else null end;
  if p_user_id is null or p_request_id is null or p_attempt_id is null or p_question_id is null
    ...
    or p_reasoning_effort is distinct from (case when p_feature in ('calculation_grading', 'drawing_analysis')
      then 'medium' else 'low' end) then
...
language sql stable security invoker set search_path = '' as $$
  ...
  select jsonb_object_agg(w.name, jsonb_build_object(
    ...
    'drawingAnalysisCount', a.drawing_analysis_count,
  ...
  from windows w cross join lateral (
    select count(*) as request_count,
      ...
      count(*) filter (where effective_status = 'completed' and feature = 'drawing_analysis') as drawing_analysis_count,
