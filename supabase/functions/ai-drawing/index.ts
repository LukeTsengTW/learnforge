...
import { createOpenAIDrawingProvider } from '../_shared/drawing-analysis.ts'
import { createDrawingHandler } from './handler.ts'
...
export default {
  fetch: withSupabase<Database>({ auth: 'user' }, async (request, ctx) => {
    ...
    return createDrawingHandler({
      userId,
      provider: key ? createOpenAIDrawingProvider(key) : null,
      ...
      async complete(ownerId, requestId, response, responseId, usage) {
        const stored = response.outcome === 'refusal'
          ...
          : { kind: response.kind, outcome: response.outcome, overallScore: response.overallScore,
            ...
            })), observations: response.observations, missingOrUnclear: response.missingOrUnclear,
