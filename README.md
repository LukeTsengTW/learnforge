...
**Current status: v0.1** — 可在瀏覽器完成的自主練習核心版本。

LearnForge 讓學生透過選擇、填空、推導與繪圖整理理解。題庫使用可版本管理的 Quiz Markdown，提交後可查看客觀題成績、自己的答案、正確／參考答案、完整解答與評分規準。不需要帳號、伺服器、資料庫或 API Key。
...
- 草稿與已提交結果保存在此瀏覽器，重新整理可恢復；提交後鎖定，重新開始會清除該次資料。
- 未完成客觀題時先警告，再由使用者決定是否提交。
- HashRouter：`#/`、`#/quiz/demo`、`#/result/demo`。
- 手機／平板／桌面排版、鍵盤可操作表單與畫布工具、文字狀態、可見 focus。

## Tech stack

保留 Vite 8、React 19、TypeScript 6 與既有 Oxlint；新增 React Router、react-markdown、remark-math、rehype-katex、KaTeX、Vitest、Testing Library、jsdom 與 ESLint。一般分層 CSS，沒有 UI framework、Redux 或網路字型。確切套件版本請看 `package-lock.json`。
...
src/
  ...
  features/quiz/            # Store / Context、Question、Result、Canvas components
...
直接閱讀 [src/content/demo.quiz.md](src/content/demo.quiz.md)。題庫以 `?raw` 匯入，再由 `quiz-loader.ts` 捕捉 `parseQuiz()` 的結果；修改題庫不需要修改 renderer。v0.1 僅註冊 demo route，尚未提供題庫清單或匯入功能。
...
Key：`learnforge:attempt:v1:<quiz-id>`；DTO 具有 `schemaVersion: 1`、quiz id、quiz revision、startedAt/updatedAt、typed answers、status。submitted 分支另有 submittedAt 與 result。

`createAttemptStore()` 使用 pure reducer，以事件同步更新並持久化，Context 搭配 `useSyncExternalStore` 訂閱。只 serialize domain DTO；提示開關、工具顏色、確認面板等 UI state 不儲存。提交會複製 answers，後續更新／再次提交被忽略；重新開始建立空白 attempt 並移除原 key，下一次作答才保存新草稿。

讀取時驗證 schema、題型、選項、日期與 stroke 座標／寬度／顏色；損壞或不相容資料顯示通知並提供新測驗。內容 fingerprint 改變時不套用舊答案。submitted score 會由已保存的答案重算，避免盲信儲存的分數。localStorage 不可用或容量不足會通知，仍可於當前頁面作答。
...
2. 至 **Actions → Deploy LearnForge to GitHub Pages → Run workflow**。
3. Workflow 執行 npm ci、lint、test，再使用 `configure-pages` 輸出的 `base_path` build，最後上傳並部署 `dist/`。

不硬編 username、repository name 或分支名稱。v0.1 未在本機推送或部署；是否啟用 push 自動部署由 repository 管理者另行設定。

## Known limitations

- 只有本機單一 demo／單次 attempt；重新開始會清除結果。不同瀏覽器、裝置或 origin 不共享進度，也不同步多分頁競爭寫入。
- 清除瀏覽器資料會失去進度；大量筆畫可能超過 localStorage 容量。斷電時尚未完成的一筆不會保存，undo/redo 不跨重新整理。
...
- 缺少多題庫管理、歷史成績、帳號與雲端同步；沒有 quiz editor 或後台。

## Future roadmap

下一個 milestone 建議 **v0.2 題庫與學習紀錄**：先確立多題庫 metadata、內容版本遷移、attempt history 與可攜式匯出，再評估帳號與同步需求。

後續可分階段規劃 regex／numeric tolerance、Supabase Auth／資料庫、AI hints、計算題參考評分、畫圖 multimodal 分析與 usage quota。這些都**沒有在 v0.1 接線或實作**。本版也不含 admin dashboard、quiz editor、cloud upload、leaderboard、social、PWA 或 SSR。
