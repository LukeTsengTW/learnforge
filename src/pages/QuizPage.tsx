...
export function QuizPage() {
  const { quiz, attempt, answerQuestion, submit, restart, storageNotice } = useAttempt()
  const [confirmIncomplete, setConfirmIncomplete] = useState(false)
  const [confirmRestart, setConfirmRestart] = useState(false)
  const navigate = useNavigate()
  if (attempt.status === 'submitted') return <Navigate replace to={`/result/${quiz.id}`} />
  ...
  function submitQuiz() { submit(); navigate(`/result/${quiz.id}`) }
  ...
  return <>
    ...
    <div className="quiz-layout"><aside className="quiz-sidebar">
      ...
      <button type="button" className="text-button restart-draft" onClick={() => { setConfirmRestart(true); window.scrollTo(0, 0) }}>重新開始測驗</button>
    </aside>
